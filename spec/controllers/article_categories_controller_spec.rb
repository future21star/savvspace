require 'rails_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

RSpec.describe ArticleCategoriesController, type: :controller do

  # This should return the minimal set of attributes required to create a valid
  # ArticleCategory. As you add validations to ArticleCategory, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) {
    skip("Add a hash of attributes valid for your model")
  }

  let(:invalid_attributes) {
    skip("Add a hash of attributes invalid for your model")
  }

  # This should return the minimal set of values that should be in the session
  # in order to pass any filters (e.g. authentication) defined in
  # ArticleCategoriesController. Be sure to keep this updated too.
  let(:valid_session) { {} }

  let(:admin_user) do
    user = FactoryGirl.create(:user, :admin)
  end

  describe "GET #index" do
    it "assigns all article_categories as @article_categories" do
      article_category = ArticleCategory.create! valid_attributes
      get :index, params: {}, session: valid_session
      expect(assigns(:article_categories)).to eq([article_category])
    end
  end

  describe "GET #show" do
    it "assigns the requested article_category as @article_category" do
      article_category = ArticleCategory.create! valid_attributes
      get :show, params: {id: article_category.to_param}, session: valid_session
      expect(assigns(:article_category)).to eq(article_category)
    end
  end

  describe "GET #new" do
    before { sign_in admin_user }

    it "assigns a new article_category as @article_category" do
      get :new, params: {}, session: valid_session
      expect(assigns(:article_category)).to be_a_new(ArticleCategory)
    end
  end

  describe "GET #edit" do
    it "assigns the requested article_category as @article_category" do
      article_category = ArticleCategory.create! valid_attributes
      get :edit, params: {id: article_category.to_param}, session: valid_session
      expect(assigns(:article_category)).to eq(article_category)
    end
  end

  describe "POST #create" do
    before { sign_in admin_user }
    
    context "with valid params" do
      it "creates a new ArticleCategory" do
        expect {
          post :create, params: {article_category: valid_attributes}, session: valid_session
        }.to change(ArticleCategory, :count).by(1)
      end

      it "assigns a newly created article_category as @article_category" do
        post :create, params: {article_category: valid_attributes}, session: valid_session
        expect(assigns(:article_category)).to be_a(ArticleCategory)
        expect(assigns(:article_category)).to be_persisted
      end

      it "redirects to the created article_category" do
        post :create, params: {article_category: valid_attributes}, session: valid_session
        expect(response).to redirect_to(ArticleCategory.last)
      end
    end

    context "with invalid params" do
      it "assigns a newly created but unsaved article_category as @article_category" do
        post :create, params: {article_category: invalid_attributes}, session: valid_session
        expect(assigns(:article_category)).to be_a_new(ArticleCategory)
      end

      it "re-renders the 'new' template" do
        post :create, params: {article_category: invalid_attributes}, session: valid_session
        expect(response).to render_template("new")
      end
    end
  end

  describe "PUT #update" do
    context "with valid params" do
      let(:new_attributes) {
        skip("Add a hash of attributes valid for your model")
      }

      it "updates the requested article_category" do
        article_category = ArticleCategory.create! valid_attributes
        put :update, params: {id: article_category.to_param, article_category: new_attributes}, session: valid_session
        article_category.reload
        skip("Add assertions for updated state")
      end

      it "assigns the requested article_category as @article_category" do
        article_category = ArticleCategory.create! valid_attributes
        put :update, params: {id: article_category.to_param, article_category: valid_attributes}, session: valid_session
        expect(assigns(:article_category)).to eq(article_category)
      end

      it "redirects to the article_category" do
        article_category = ArticleCategory.create! valid_attributes
        put :update, params: {id: article_category.to_param, article_category: valid_attributes}, session: valid_session
        expect(response).to redirect_to(article_category)
      end
    end

    context "with invalid params" do
      it "assigns the article_category as @article_category" do
        article_category = ArticleCategory.create! valid_attributes
        put :update, params: {id: article_category.to_param, article_category: invalid_attributes}, session: valid_session
        expect(assigns(:article_category)).to eq(article_category)
      end

      it "re-renders the 'edit' template" do
        article_category = ArticleCategory.create! valid_attributes
        put :update, params: {id: article_category.to_param, article_category: invalid_attributes}, session: valid_session
        expect(response).to render_template("edit")
      end
    end
  end

  describe "DELETE #destroy" do
    it "destroys the requested article_category" do
      article_category = ArticleCategory.create! valid_attributes
      expect {
        delete :destroy, params: {id: article_category.to_param}, session: valid_session
      }.to change(ArticleCategory, :count).by(-1)
    end

    it "redirects to the article_categories list" do
      article_category = ArticleCategory.create! valid_attributes
      delete :destroy, params: {id: article_category.to_param}, session: valid_session
      expect(response).to redirect_to(article_categories_url)
    end
  end

end

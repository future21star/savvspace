...
call_reg = /^a call should be setup between the agent "([^"]*)" and me at "([^"]*)"$/
Then(call_reg) do |profile_username, phone_number|
  profile = Profile.find_by(username: profile_username)
  expect(profile.phone_calls.from_number(phone_number).exists?).to be true
end

call_reg = /^I've placed a call to agent "([^"]*)" from my phone number "([^"]*)"$/
Given(call_reg) do |profile_username, phone_number|

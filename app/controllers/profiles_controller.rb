class ProfilesController < ApplicationController
  ...
  def show
    authorize @profile
    @phone_call = PhoneCall.new(profile: @profile) if @profile.phone
  ...
  def destroy
    ...
    @profile.destroy
    respond_to do |format|
      format.html { redirect_to profiles_url, notice: t('.notice') }
  ...
  def profile_params
    params.require(:profile)
          .permit(:name, :username, :bio, :contact_email, :linked_in, :facebook,
                  :twitter, :instagram, :avatar, :background,
                  phone_attributes: [:number], past_avatars: [])
  end
  ...
  # TODO: refactor me so this cop can be re-enabled
  ...
  def set_past_avatar
    if profile_params[:avatar].instance_of? String
      ...
      if past_avatar.present?
        ...
        att_file = Cloudinary::Uploader
                   .upload(Cloudinary::Utils.cloudinary_url(@profile.avatar.path))
        new_past_avatar = @profile.past_avatars.new(public_id: att_file['public_id'],
                                                    version: att_file['version'],
                                                    width: att_file['width'],
                                                    height: att_file['height'],
                                                    format: att_file['format'],
                                                    resource_type: att_file['resource_type'])

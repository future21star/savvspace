...
class RetsRabbitV1MlsAdapter < MlsAdapter
  ...
  format :json
  headers 'Authorization' => "Bearer #{RetsRabbitV1MlsAdapter.access_token}",
  ...
  def property_list(search)
    ...
    Rails.cache.fetch("rets_rabbit_v1_property_search_#{query.hash}",
  ...
  def listing(mls_server, id)
    build_property(mls_server,
                   self.class.get("/v1/#{mls_server.server_hash}/listing/#{id}")
                             .parsed_response)
  end
  ...
  def open_houses_list(mls, _offset = 0, _result_set = [])
    results = self.class.get("/v1/#{mls.server_hash}/open_house/search",
                             query: {
                               'offset' => 0,
                               'EVENT100:date' => Time.zone.today.to_s + '+'
                             })
  ...
  def fetch_property_list(mls_server, offset = 0, _result_set = [])
    ...
    if (response.parsed_response['total_records'] > result_set.size) &&
       response.parsed_response['results'].empty?
      Rails.logger
           .debug("#{response.parsed_response['total_records'] - result_set.size} more to go...")
  ...
  def build_property(mls, struct)
    Property.new(mls_server_id: mls.id,
                 ...
                 mls_updated_at: Time.zone.parse(struct['fields']['LIST_87']))
  end
